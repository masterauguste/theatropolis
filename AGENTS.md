...
- Release freshness note: release-version catalogs are never cached; every automatic load and explicit Settings “Check again” action makes a fresh upstream request. Theatropolis entries still require both Linux archives plus `checksums.txt`, so an in-progress tag publication is not exposed as installable.
