...
- `internal/control/observed.go`, `probe.go`, `probe_scheduler.go` — pool address sources: the observed control-connection address (XFF last element via Caddy's default `X-Forwarded-For`, fallback gRPC peer addr), `RequestAddressProbe` + `address_probe_report` handling (probe command gated on online + `address-probe-v1`), and a 5 min scheduler (`SetProbeInterval`, `Server.Close` for tests) that re-probes ONLY (agent, family) pairs actively consumed with `AddressSourceForFamily` == probed. Pool address hierarchy per family: override → observed → probed → interface-reported; refs may pin `"family"` (auto/ipv4/ipv6). Only globally routable addresses are used (no RFC1918/ULA/CGNAT/reserved — TEST-NET/documentation ranges deliberately kept), from every source including the override; each package enforces this with its own `globallyRoutable` helper (agent collection, master sanitize/observed/probe-report checks, registry setters as the last gate: `SetReported`/`SetProbed` drop, `SetObserved` ignores, `SetOverride` rejects with `ErrInvalidAddress`).
