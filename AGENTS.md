...
- Configuration deploy submissions use a fetch/JSON status flow so the first deployment stays on the editor page while validation and activation finish. The guided TLS editor's self-signed mode emits reserved relative certificate paths; it never asks the master to generate or retain the private key.
